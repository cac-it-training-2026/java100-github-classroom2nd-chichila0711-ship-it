...
class Robot {
	...
	void setWater(int water) {
		//		this.water→フィールドwater(自分の中の変数）　　←代入　　　　water→引数（外から渡された値）
		this.water = water;
	}
	...
	void makeOmelet(int eggNum, int butterNum) {
		int omeletcount1 = eggNum / 2;
		int omeletcount2 = butterNum / 5;

		if (omeletcount1 > omeletcount2) {
			System.out.println("\n" + omeletcount2 + "人分のオムレツを作成しました。\n");
		} else {
			System.out.println("\n" + omeletcount1 + "人分のオムレツを作成しました。\n");
		}
	}
	...
	String makeEggDishes(int flourNum, int sugarNum, int eggNum, int butterNum) {
		...
		String menu = null;
		if (flourNum >= 0170 && sugarNum >= 50 && eggNum >= 1 && butterNum >= 80) {
			menu = "クッキー";
